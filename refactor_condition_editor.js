...
// 1. Add state for rhsFieldPickerOpen
...
// 2. Add rhsSelectField function next to selectField
...
// 3. Inject the RHS Type Toggle and Field Picker in renderValueInput
...
// 4. Update the JSX where renderValueInput is called
      ...
      <div className="flex items-center gap-1">
        {field?.type !== "boolean" && field?.type !== "enum" && condition.operator !== "in" && condition.operator !== "between" && (
          <Select value={condition.rhsType || "STATIC"} onValueChange={(v) => onChange({ rhsType: v as any, value: "" })}>
            <SelectTrigger size="sm" className="h-8 w-14 px-2 border-0 bg-transparent text-muted-foreground hover:bg-muted/50" showChevron={false} title="Toggle Static Value or Field Reference">
...
console.log("Refactored condition-editor.tsx");
