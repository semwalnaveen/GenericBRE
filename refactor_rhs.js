...
const selectFieldReplacement = `  const selectField = (key: string, sourceType?: "BUSINESS_FIELD" | "RULE_OUTPUT", sourceRuleId?: string) => {
    recordRecentField(key);
    onChange({ field: key, value: "", value2: undefined, sourceType, sourceRuleId });
    setFieldPickerOpen(false);
  };
  ...
  const selectRhsField = (key: string, rhsSourceType?: "BUSINESS_FIELD" | "RULE_OUTPUT", rhsSourceRuleId?: string) => {
    onChange({ value: key, rhsSourceType, rhsSourceRuleId });
    setRhsFieldPickerOpen(false);
  };`;
code = code.replace(selectFieldBlock, selectFieldReplacement);

// Add renderRhsFieldPicker
const renderValueInputStart = `  const renderValueInput = () => {`;
const renderRhsFieldPickerBlock = `  const renderRhsFieldPicker = () => {
...
code = code.replace(renderValueInputStart, renderRhsFieldPickerBlock);

// Replace the value input area in the return block
const valueAreaStart = `{field?.unit && <span className="text-sm text-muted-foreground">{field.unit}</span>}`;
const newValueAreaStart = `      <div className="flex items-center gap-1">
...
code = code.replace(`      {renderValueInput()}\n      {field?.unit && <span className="text-sm text-muted-foreground">{field.unit}</span>}`, newValueAreaStart);
// just in case they are separated differently
code = code.replace(`      {renderValueInput()}
      {field?.unit && <span className="text-sm text-muted-foreground">{field.unit}</span>}`, newValueAreaStart);

fs.writeFileSync("src/components/rule-builder/condition-editor.tsx", code, "utf8");
console.log("RHS Script done.");
